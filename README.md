...
Monthly industrial food production is analyzed and forecast using advanced time-series techniques in R. This repository (currently centered on the notebook `Times_Series_Assesment.ipynb`) provides a reproducible workflow: data preparation, exploratory analysis, decomposition, structural break detection, ARIMA-based modeling, and forward forecasting.
...
## 5. Setup & Reproducibility
You can work in either R directly or via the Jupyter notebook interface (with an R kernel). Required R packages:
- `forecast`
- `tseries`
- `psych`
- `strucchange`
